...
function initAll() {
  window.AJAX({
    ...
    success(data) {
      ...
      basicData.leftEmployee = data.leftEmployee
  ...
  document.querySelector('#menu').addEventListener('click', function (e) {
    ...
    switch (target) {
      ...
      case 'reset':
        ...
        basicData.leftEmployee = Object.assign([], basicData.employee)
...
function lottery() {
  ...
  rotateBall().then(() => {
    ...
    if (leftCount < perCount) {
      ...
      basicData.leftEmployee = basicData.employee.slice()
      leftCount = basicData.leftUsers.length
    }
    ...
    for (let i = 0; i < perCount; i++) {
      ...
      if (currentPrizeIndex <= 2) {
        luckyId = random(basicData.leftEmployee.length)
      } else {
        luckyId = random(leftCount)
      }

      let ers = basicData.leftUsers.splice(luckyId, 1)[0]
      let yee = basicData.leftEmployee.splice(luckyId, 1)[0]
      if (currentPrizeIndex <= 2) {
        currentLuckys.push(yee)
      } else {
        currentLuckys.push(ers)
      }
